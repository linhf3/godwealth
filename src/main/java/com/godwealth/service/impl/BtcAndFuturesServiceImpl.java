...
public class BtcAndFuturesServiceImpl implements BtcAndFuturesService {
    ...
    @Autowired
    private FuturesCoreAlgorithm futuresCoreAlgorithm;
    ...
    private BtcEntity getData(FuturesData futuresData) throws IOException {
        ...
        Map<String, Object> map = futuresCoreAlgorithm.deviationTheDayRate(mapd);
        System.out.println("偏离"+map);
        ...
        //当前价格
        Map<String,String> priceMap =  transformationCollection((List) mapd.get("trends"),priceLinkListAll);
        priceLinkListAll = priceLinkListAll.stream().distinct().collect(Collectors.toList());
        Collections.sort(priceLinkListAll);
        //涨幅
        Double p = Double.valueOf(priceMap.get("oPrice"));
        Double cPrice = Double.valueOf(priceMap.get("cPrice"));
        double zhangfu = (cPrice-pre)/pre*100;
        btcEntity.setZhangfu(new StringBuilder(df.format(zhangfu)).append("%").toString());
        //当前价格
        btcEntity.setPrice(Double.valueOf(priceMap.get("cPrice")));
        //波动
        ...
        btcEntity.setProportion(proportion);
        String d = proportion.substring(1, proportion.length() - 1);
        if (d.startsWith(".")){
            d = new StringBuilder("0").append(d).toString();
        }
        btcEntity.setProportionAbs(Math.abs(Double.valueOf(d))>=90?"1":"0");
        return btcEntity;
    }

    private Map<String,String> transformationCollection(List trends, List<Double> priceLinkListAll) {
        ...
        for (int i = 0; i < trends.size(); i++) {
            ...
            if (i == 0){
                map.put("oPrice",split[1]);
            }
            if (i == trends.size()-1){
                map.put("cPrice",split[1]);
                return map;
            }
        }
        return null;
